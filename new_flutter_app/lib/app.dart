...
class _FitNovaAppState extends State<FitNovaApp> {
  ...
  Future<void> _restoreSession() async {
    ...
    var valid = token != null && _hasValidJwtExpiry(token);

    // The access token is short-lived. If it has expired but the
    // refresh token is still valid, restore the session instead of
    // immediately sending the user to the login screen.
    if (!valid) {
      try {
        await _auth.refreshStoredSession();

        token = await _storage.getToken();
        valid = token != null && _hasValidJwtExpiry(token);
      } catch (_) {
        ...
        builder: (_, accent, __) => MaterialApp(
          ...
          routes: {
            ...
            AppConstants.emailVerificationRoute: (context) {
              ...
              return EmailVerificationScreen(
                ...
                pendingProfile: data['profile'] is UserProfile
                    ...
                    : null,
                developmentCode:
                    data['developmentCode'] as String?,
              );
            },
            ...
            AppConstants.homeRoute: (context) {
              ...
              return FitNovaShell(
                api: _api,
                initialProfile: arguments is UserProfile
                    ? arguments
                    : _initialProfile,
                onLogout: _signOut,
              );
            ...
            AppConstants.profileRoute: (_) => ProfileScreen(
                  profileService:
                      ProfileService(apiClient: _api),
                  onLogout: _signOut,
                ),
            AppConstants.notificationsRoute: (_) =>
                NotificationsScreen(
                  notificationService:
                      NotificationService(apiClient: _api),
                ),
            ...
            AppConstants.mealPlanRoute: (_) => MealPlanScreen(
                  mealPlanService:
                      MealPlanService(apiClient: _api),
                ),
            AppConstants.workoutPlanRoute: (_) =>
                WorkoutPlanScreen(
                  workoutPlanService:
                      WorkoutPlanService(apiClient: _api),
                  initialProfile: _initialProfile,
                ),
            AppConstants.progressRoute: (_) =>
                ProgressPage(
                  service: ProgressService(apiClient: _api),
                ),
          },
        ),
      );
          ...
          child: ListView(
            ...
            children: [
              ...
              const _OnboardingHint(
                ...
                text:
                    'AI plans that evolve with your progress',
              ),
              const _OnboardingHint(
                icon: Icons.restaurant,
                text:
                    'Simple nutrition that fits your lifestyle',
              ),
              const _OnboardingHint(
                icon: Icons.insights,
                text:
...
class FitNovaShell extends StatefulWidget {
  ...
  @override
  State<FitNovaShell> createState() =>
      _FitNovaShellState();
}
...
class _FitNovaShellState extends State<FitNovaShell> {
  ...
  late final _profile =
      ProfileService(apiClient: widget.api);

  late final _progress =
      ProgressService(apiClient: widget.api);

  late final _workouts =
      WorkoutPlanService(apiClient: widget.api);

  late final _meals =
      MealPlanService(apiClient: widget.api);

  late final _chat =
      ChatService(apiClient: widget.api);

  late final _voice =
      VoiceService(apiClient: widget.api);
  ...
  Future<void> _maybeStartTour() async {
    final show =
        await DashboardTourOverlay.shouldShow();
  ...
  Widget build(BuildContext context) =>
      FutureBuilder<List<Object?>>(
        ...
        builder: (context, snapshot) {
          ...
          final p =
              snapshot.data![0] as UserProfile;
          final s =
              snapshot.data![1] as ProgressStats;
          ...
          return _Page(
            ...
            trailing: _BackendStatusDot(
              online: widget.backendOnline,
            ),
            children: [
              _HeroCard(
                title: 'Today’s focus',
...
class _ProgressPageState
  ...
  @override
  void didUpdateWidget(
    covariant ProgressPage oldWidget,
  ) {
    super.didUpdateWidget(oldWidget);
  ...
  void _refresh() {
    _load =
        widget.service.getProgressStats();
          ...
          return _Page(
            ...
            children: [
              _HeroCard(
                ...
                value:
                    '${s.avgGoalCompletion?.toStringAsFixed(0) ?? '—'}%',
                detail:
                    '${s.totalEntries} check-ins recorded',
                icon: Icons.insights,
              ),
              const SizedBox(height: 18),
              Row(
              ...
              _ListCard(
                ...
                title:
                    '${s.totalWorkouts} Workouts Completed',
                detail:
                    '${s.workoutStreak}-Day Current Streak',
              ),
              _ListCard(
                icon:
...
class _CoachPageState
    extends State<CoachPage> {
  ...
  Future<String> _getSessionId() async {
    final userId =
        await _userIdService.getCurrentUserId();
  ...
  Widget build(BuildContext context) =>
      _Page(
        title: 'AI Coach',
        subtitle:
            'Ask FitNova AI anything about your goals.',
        children: [
          if (_messages.isEmpty)
            const _ListCard(
              ...
              detail:
                  'Your coach learns from every conversation.',
            ),
          ..._messages.map(
            (m) => Align(
              alignment: m.isUser
      ...
      FutureBuilder<UserProfile>(
        ...
        builder: (context, snapshot) {
          ...
          return _Page(
            title: 'Profile',
            subtitle:
                'Make FitNova AI work for your lifestyle.',
            children: [
              Card(
                child: ListTile(
                  contentPadding:
...
class _LoadingPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _Page(
        title: title,
        subtitle:
            'FitNova AI is syncing securely with your account.',
        children: const [
          Center(
            child: Padding(
